...
![Market Report](graphics/Performance_Analysis/IMAG-Market-Report-Performance.png){#fig-imag-market-report}
...
The simulation's performance analysis tools enable the same cycle we described in the chapter. Measurement establishes what happened. Attribution reveals why it happened. Appraisal determines what should change. If the attribution shows that our tactical tilts consistently detracted, the process for generating those tilts needs review. If the strategic allocation analysis shows misalignment with client targets, the strategic weights need adjustment. If the competitive metrics show underperformance relative to peers, the entire investment process must be examined.

The discipline is the same one we applied to the Schmidt-Miller family. Performance analysis is not about celebrating wins or lamenting losses. It is about understanding outcomes, communicating them clearly, and using that understanding to improve future decisions. The simulation provides the data. The framework from this chapter provides the method. The quality of our analysis determines whether the cycle closes productively or spins without learning.